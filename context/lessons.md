...
- 2026-04-19 (docs): A documentation restructure benefits from one-commit-per-phase discipline: each phase is independently reviewable, and later phases (README rewrite) depend on earlier ones (individual docs existing) so the commit log doubles as the diff review surface.
- 2026-04-19 (cli): Help-text regressions are best locked at the CliRunner output level so user-facing capability language can evolve without coupling tests to Typer internals.
- 2026-04-19 (release): The `chronicler` PyPI name was taken; always verify package name availability before finalizing release infrastructure. Distribution name and CLI entry point are independent — `pip install chronicler-ttrpg` can still give you the `chronicler` command.
- 2026-04-19 (release): Black formatting failures will block the publish job since `publish` depends on `test`. Run `uv run black --check src/ tests/` locally before pushing a release tag or the tag needs to be deleted and re-pushed.
- 2026-04-19 (release): PyPI OIDC trusted publisher setup is a one-time manual web UI step (Owner, Repository, Workflow, Environment fields must match exactly). Once configured, tagging `v*.*.*` is the only step needed to ship a new release.
