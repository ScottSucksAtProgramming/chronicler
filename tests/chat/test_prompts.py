...
class TestBuildChatPrompt:
    def test_includes_core_vault_context_and_supporting_notes(self, sample_results, sample_history):
        prompt = build_chat_prompt(
            question="What do we know about the ship?",
            core_notes=[
                DirectVaultNote(path="Party/Seven.md", content="Seven is a wizard in the party."),
                DirectVaultNote(path="_Agent/Memory/vault-guide.md", content="Party/ is authoritative for player characters."),
            ],
            supporting_notes=[
                DirectVaultNote(path="Sessions/Session-002.md", content="The party boarded the Black Cherry."),
            ],
    ...
    def test_direct_vault_notes_are_described_as_more_authoritative(self, sample_results, sample_history):
        prompt = build_chat_prompt(
            question="Tell me about Seven.",
            core_notes=[DirectVaultNote(path="Party/Seven.md", content="Seven is a wizard.")],
            supporting_notes=[],
            retrieval_results=sample_results,
            conversation_history=sample_history,
        )
    ...
    def test_prompt_requires_explicit_fact_vs_inference_language(self, sample_results, sample_history):
        prompt = build_chat_prompt(
    ...
    def test_prompt_requires_definition_questions_to_admit_missing_direct_note(self, sample_results, sample_history):
        prompt = build_chat_prompt(
            question="What is the Black Cherry?",
            core_notes=[],
            supporting_notes=[],
